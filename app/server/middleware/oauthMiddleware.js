...
const registerWebhooks= async({session, webhookData})=>{
  ...
  try {
    await axios.post(`https://${session.shop}/admin/api/${process.env.API_VERSION}/webhooks.json`, webhookData, {
      headers: {
    ...
    console.error('Error registering app uninstalled webhook:', error.response ? error.response.data : error.message);
  }finally{
...
export const oauthCallback = async (req, res) => {
  ...
  try {
    ...
    const session = await prisma.session.upsert({
      ...
      create: { shop, accessToken:access_token, scope, state, id:`offline_${shop}` }
    });
